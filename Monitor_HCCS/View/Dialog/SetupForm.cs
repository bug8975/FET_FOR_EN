    ...
    public partial class SetupForm : DevExpress.XtraEditors.XtraForm, ISetupView
    {
        ...
        private void txButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ShowToast_InSetup("Notice", "The NetWork is Not Connected，you need to connect the WIFI firstly", 2);
                    return;
                }

                if (Monitor_HCCS.Service.BLECode.GetIntance.CurrentService == null)
                {
                    ShowToast_InSetup("Notice", "The Bluetooth is Not Connected，you need to connect the bluetooth firstly", 2);
                    return;
                }
                simpleButton3.Text = "Check";
                simpleButton3.Visible = false;
                if (labelControl3.Text.Equals(labelControl4.Text))
                {
                    ShowToast_InSetup("Notice", "The Current Version is the latest！", 2);
        ...
        {
            if (this.IsHandleCreated)
                this.Invoke(new Action(() => { labelControl4.Text = msg; simpleButton3.Text = "upgrading"; simpleButton3.Visible = true; }));
        }
        ...
        //判断网络状况的方法,返回值true为连接，false为Not Connected  
